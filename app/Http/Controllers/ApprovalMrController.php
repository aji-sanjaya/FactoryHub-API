...
class ApprovalMrController extends Controller
{
    ...
    {
        $approvalConfig = config('idempiere.approval-mr');
        ...
        $query = MInOut::query()
            ->select(
                'm_inout.*',
                DB::raw("(SELECT cb.name FROM c_bpartner cb WHERE cb.c_bpartner_id = m_inout.c_bpartner_id) as bpartner_name")
            );
        ...
        $query->addSelect([
            'is_my_approval' => DB::connection('idempiere')
                ...
                ->whereColumn('p.record_id', 'm_inout.m_inout_id')
        ...
        if ($clientId) {
            $query->where('m_inout.ad_client_id', $clientId);
        }

        if ($orgId && $orgId > 0) {
            $query->where('m_inout.ad_org_id', $orgId);
        }
        ...
        $statsQuery = MInOut::query()->where('issotrx', $approvalConfig['defaults']['is_so_trx']);
        if ($clientId)
        ...
        if ($status !== $approvalConfig['defaults']['all_filter_value']) {
            if (isset($statusConfig['filter_aliases'][$status])) {
                $query->whereIn('m_inout.docstatus', $statusConfig['filter_aliases'][$status]);
            } else {
                $query->where('m_inout.docstatus', $status);
            }
        }
        ...
        if ($request->has('search') && $request->search) {
            ...
            $query->where(function ($q) use ($term) {
                $q->where('m_inout.documentno', 'ilike', "%{$term}%")
                    ->orWhere('m_inout.description', 'ilike', "%{$term}%")
                    ->orWhereExists(function ($sub) use ($term) {
                        $sub->select(DB::raw(1))
                            ->from('c_bpartner as cbsearch')
                            ->whereColumn('cbsearch.c_bpartner_id', 'm_inout.c_bpartner_id')
        ...
        if ($request->has('c_bpartner_id') && $request->c_bpartner_id) {
            $query->where('m_inout.c_bpartner_id', $request->c_bpartner_id);
        }

        $receipts = $query->orderBy('m_inout.movementdate', 'desc')->paginate($approvalConfig['limits']['list_per_page']);

        if ($request->ajax()) {
            return view('pages.approval-mr.partials.table', compact('receipts'));
        }

        return view('pages.approval-mr.index', compact(
            'receipts',
            'countPending',
            'countApproved',
            'countRejected',
    ...
    public function show($id)
    {
        ...
        try {
            ...
            $receipt = MInOut::find($decryptedId);
        } catch (\Exception $e) {
            // Fallback for non-encrypted IDs (dev/test)
            $receipt = MInOut::find($id);
        }
        ...
        if (request()->ajax() && request()->has('ajax_tab')) {
            ...
            if ($tab === 'attachments') {
                ...
                return view('pages.approval-mr.partials.tab-attachments', [
        ...
        $isMyApproval = DB::connection('idempiere')
            ...
            ->join('ad_wf_activity as a', 'a.ad_wf_process_id', '=', 'p.ad_wf_process_id')
            ->join('ad_wf_responsible as wr', 'wr.ad_wf_responsible_id', '=', 'a.ad_wf_responsible_id')
            ->where('p.record_id', $receipt->m_inout_id)
        ...
        // Basic View Data
        return view('pages.approval-mr.show', [
            'receipt' => $receipt,
            'encryptedId' => Crypt::encryptString($receipt->m_inout_id),
            'title' => 'Approval MR: ' . $receipt->documentno,
    ...
    public function process(Request $request, $id)
    {
        ...
        if ($response->successful()) {
            ...
            try {
                ...
                if (!$receipt || is_null($receipt->adw_checked_date)) {
                    // Step 1: Checked
                    $updateData = [
