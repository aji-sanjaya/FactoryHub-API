...
class SalesOrderLineController extends Controller
{
    public function create(Request $request)
    {
        ...
        if (!$documentId) {
            return redirect()->route('sales-order.index')->with('error', 'Document ID required.');
        }
        ...
        try {
            ...
            $salesOrder = \Illuminate\Support\Facades\DB::connection('idempiere')->table('c_order')
            ...
            return redirect()->route('sales-order.index')->with('error', 'Invalid Document ID.');
        }

        // Return View with Encrypted ID
        return view('pages.sales-order.line_form', [
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('sales-order.index', ['document_id' => $documentId, 'tab' => 'lines'])
    ...
    public function downloadTemplate()
    {
        try {
            ...
            $filename = 'sales-order-line-template-' . date('Y-m-d') . '.xlsx';
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            if (count($failedRows) > 0) {
                ...
                $errorFileUrl = route('sales-order.line.download-errors', ['filename' => $errorFileName]);
            }
