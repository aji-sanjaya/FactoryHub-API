...
<div class="overflow-hidden rounded-b-2xl border border-t-0 border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="max-w-full overflow-x-auto">
        <table class="w-full table-auto">
            ...
            <tbody>
                @forelse ($deliverySchedules as $index => $req)
                    <tr class="border-t border-gray-200 dark:border-gray-800">
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            {{ $index + 1 + ($deliverySchedules->currentPage() - 1) * $deliverySchedules->perPage() }}
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('delivery-schedule.index', ['document_id' => \Illuminate\Support\Facades\Crypt::encryptString($req->c_order_id)]) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500">
                                ...
                                <button type="button"
                                    onclick="openPrintModal('{{ route('delivery-schedule.print', ['id' => \Illuminate\Support\Facades\Crypt::encryptString($req->c_order_id)]) }}')"
                        ...
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            {{ $req->datepromised ? date('d M Y', strtotime($req->datepromised)) : '-' }}
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
    ...
    <div class="px-4 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-sm text-gray-500 dark:text-gray-400">
            Showing <span class="font-medium text-gray-900 dark:text-white">{{ $deliverySchedules->firstItem() ?? 0 }}</span> 
            to <span class="font-medium text-gray-900 dark:text-white">{{ $deliverySchedules->lastItem() ?? 0 }}</span> 
            of <span class="font-medium text-gray-900 dark:text-white">{{ $deliverySchedules->total() }}</span> results
        </div>
        <div class="w-full sm:w-auto"> 
                {{ $deliverySchedules->onEachSide(1)->links('pagination::tailwind') }}
        </div>
    </div>
</div>
