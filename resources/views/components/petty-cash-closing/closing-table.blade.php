...
<div class="overflow-hidden rounded-b-2xl border border-t-0 border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="max-w-full overflow-x-auto">
        <table class="w-full table-auto">
            ...
            <tbody>
                @forelse ($closings as $index => $request)
                    <tr class="border-t border-gray-200 dark:border-gray-800">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('petty-cash-closing.index', ['document_id' => \Illuminate\Support\Facades\Crypt::encryptString($item->adw_pettycash_closing_id)]) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500">
                        ...
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            {{ \Carbon\Carbon::parse($item->datetrx)->format('Y-m-d') }}
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            {{ $item->user->name ?? '-' }}
                        </td>
    ...
    <div class="px-4 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-sm text-gray-500 dark:text-gray-400">
            Showing <span class="font-medium text-gray-900 dark:text-white">{{ $closings->firstItem() ?? 0 }}</span> 
