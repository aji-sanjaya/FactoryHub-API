...
<div class="overflow-hidden rounded-b-2xl border border-t-0 border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <tr class="border-t border-gray-200 dark:border-gray-800">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('purchase-order.index', ['document_id' => \Illuminate\Support\Facades\Crypt::encryptString($order->c_order_id)]) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500">
                                ...
                                @if($order->docstatus !== 'VO')
                                    <button type="button"
                                        onclick="openPrintModal('{{ route('purchase-order.print', ['id' => \Illuminate\Support\Facades\Crypt::encryptString($order->c_order_id)]) }}')"
                        ...
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                             {{ date('d M Y', strtotime($order->dateordered)) }}
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            <div class="truncate max-w-xs whitespace-nowrap" title="{{ $order->description }}">
                                {{ $order->description ?: '-' }}
                            </div>
                        </td>
    ...
    <div class="px-4 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-sm text-gray-500 dark:text-gray-400">
            Showing <span class="font-medium text-gray-900 dark:text-white">{{ $orders->firstItem() ?? 0 }}</span> 
            to <span class="font-medium text-gray-900 dark:text-white">{{ $orders->lastItem() ?? 0 }}</span> 
            of <span class="font-medium text-gray-900 dark:text-white">{{ $orders->total() }}</span> results
        </div>
