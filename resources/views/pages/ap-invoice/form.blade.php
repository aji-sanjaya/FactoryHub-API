...
@section('content')
            ...
            <div class="flex gap-3">
                ...
                <div id="header-actions" class="{{ $activeTab == 'header' ? 'block' : 'hidden' }}">
                    ...
                    @if(!$isNew && $invoice->docstatus !== 'VO')
                        <button onclick="openInvoicePrintModal('{{ route('ap-invoice.print', ['id' => \Illuminate\Support\Facades\Crypt::encryptString($invoice->c_invoice_id)]) }}')"
                    ...
                    @if(in_array($cs, $completeFromStatuses, true))
                        <button onclick="processDocument('CO')"
                            ...
                            <div>
                                ...
                                <div class="text-xs text-green-600 dark:text-green-400">{{ $workflowActionDescriptions['CO'] ?? 'Process and complete this invoice' }}</div>
                            </div>
                        </button>
                    @endif
                    @if(in_array($cs, $voidFromStatuses, true))
                        <button onclick="processDocument('VO')"
                            ...
                            <div>
                                ...
                                <div class="text-xs text-red-600 dark:text-red-400">{{ $workflowActionDescriptions['VO'] ?? 'Void this invoice document' }}</div>
                            </div>
                        </button>
                    @endif
                    @if(in_array($cs, $reverseFromStatuses, true))
                        <button onclick="processDocument('RC')"
                            ...
                            <div>
                                ...
                                <div class="text-xs text-orange-600 dark:text-orange-400">{{ $workflowActionDescriptions['RC'] ?? 'Reverse this completed invoice' }}</div>
                            </div>
                        </button>
                    @endif
                    @if(in_array($cs, $closeFromStatuses, true))
                        <button onclick="processDocument('CL')"
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-600 dark:text-gray-400">{{ $workflowActionDescriptions['CL'] ?? 'Close this invoice document' }}</div>
                            </div>
                        </button>
                    @endif
                </div>
    ...
    @push('scripts')
        ...
        function initScripts() {
            $('#c_bpartner_id, #org_id, #doc_type_id, #c_currency_id, #c_paymentterm_id, #c_project_id, #c_tax_id, #ad_user_id, #adw_ad_user_approved_id, #adw_ad_user_verification_id, #c_department_id, #c_costcenter_id').select2({
                width: '100%', placeholder: '- Select -'
            });
            $('#c_project_id').select2({ width: '100%', placeholder: '- None -', allowClear: true });
            $('#adw_ad_user_approved_id').select2({ width: '100%', placeholder: '- Select User -', allowClear: true });
            $('#adw_ad_user_verification_id').select2({ width: '100%', placeholder: '- Select User -', allowClear: true });
        ...
        function submitHeader() {
            ...
            const payload = {
                ...
                adw_ad_user_verification_id: $('#adw_ad_user_verification_id').val() || null,
