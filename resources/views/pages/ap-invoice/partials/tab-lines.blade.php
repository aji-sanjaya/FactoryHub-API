...
<div id="receiptSelectionModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm" onclick="closeReceiptModal()"></div>
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col z-10"
            ...
            <div
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Select from Receipt</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Only completed receipts are shown. Type
            ...
            <div
                ...
                <div class="flex gap-3">
                    <div class="relative flex-1">
                        ...
                        <input type="text" id="receipt_search_input" placeholder="Search by Receipt No or Product..."
                            ...
                            oninput="debounceReceiptSearch(this.value)">
                    </div>
                    <button type="button" onclick="loadReceiptLines(1)"
            ...
            <div class="flex-1 overflow-y-auto" style="min-height:200px;">
                <div id="receipt_loading" class="hidden py-16 text-center">
                ...
                <div id="receipt_empty" class="py-16 text-center">
                    <p class="text-sm text-gray-500">Type at least 2 characters to search receipt lines.</p>
                </div>
                <table class="w-full text-sm" id="receipt_table" style="display:none;">
                    <thead
                        class="sticky top-0 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-10">
                        <tr>
                            <th class="w-10 px-4 py-3">
                                <input type="checkbox" id="receipt_select_all" onchange="onReceiptSelectAllChange(this)"
                    ...
                    <tbody id="receipt_table_body" class="divide-y divide-gray-100 dark:divide-gray-800"></tbody>
                </table>
            </div>
            <!-- Pagination -->
            <div
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div class="text-xs text-gray-500" id="receipt_pagination_info">&nbsp;</div>
                    <div class="flex items-center gap-1" id="receipt_pagination_btns"></div>
                </div>
            </div>
            <!-- Footer -->
            <div
                class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between flex-shrink-0">
                <span id="receipt_selected_count" class="text-sm text-gray-500">No line selected</span>
                <div class="flex gap-3">
                    <button type="button" onclick="closeReceiptModal()"
                    ...
                    <button type="button" onclick="confirmReceiptAdd()" id="receipt_confirm_btn" disabled
...
@if($canEdit)
    <div id="lines-create-form" class="hidden">
        <div class="bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
            ...
            <form id="createLineForm" onsubmit="event.preventDefault(); saveLine();" class="space-y-6">
                ...
                {{-- Receipt line link (set by Modal) --}}
                ...
                {{-- Linked Receipt info banner (shown via JS) --}}
                <div id="receipt_info_container"
                    ...
                    <label class="block text-sm font-medium text-blue-800 dark:text-blue-300 mb-1">Linked
                        Receipt</label>
                    <div class="flex items-center gap-2">
                        <span id="receipt_info_text" class="text-sm text-blue-900 dark:text-blue-200 font-medium"></span>
                    </div>
                </div>
    ...
    <script>
        (function () {
            ...
            const STORE_URL = '{{ route("ap-invoice.line.store") }}';
            const UPD_URL = '{{ route("ap-invoice.line.update") }}';
            const DEL_URL = '{{ route("ap-invoice.line.delete") }}';
            const GR_URL = '{{ route("ap-invoice.api.gr-lines") }}';
            const PROD_URL = '{{ route("ap-invoice.api.products") }}';
                ...
                if ($('#gr_line_id').length && !$('#gr_line_id').data('select2')) {
                    $('#gr_line_id').select2({
                        ...
                        ajax: {
                            ...
                            data: p => ({ q: p.term, page: p.page || 1, vendor_id: $('#c_bpartner_id').val() }),
                            processResults: d => ({ results: d.results, pagination: { more: d.pagination.more } }),
                            cache: false
                        }
                    });
            ...
            window.clearReceiptLink = function () {
                ...
                const container = document.getElementById('receipt_info_container');
            ...
            window.editLine = function (lineId, productName, productId, qty, price, desc, lineNo, uomName, mInOutLineId, receiptNo, poRef, receiptDate) {
                ...
                // Restore linked receipt info if available
                if (mInOutLineId) {
                    document.getElementById('m_inoutline_id').value = mInOutLineId;
                    const infoContainer = document.getElementById('receipt_info_container');
                    const infoText = document.getElementById('receipt_info_text');
                    if (infoContainer && infoText) {
                        let text = receiptNo;
                        if (poRef) text += ` (PO: ${poRef})`;
                        if (receiptDate) text += ` - ${receiptDate}`;
                        infoText.textContent = text;
                        infoContainer.classList.remove('hidden');
                    }
                    // Disable product selection when linked to a receipt
                    if (typeof $ !== 'undefined') {
