        ...
        <table class="w-full table-auto">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($invoices as $invoice)
                    <tr
                        class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors {{ isset($invoice->is_my_approval) && $invoice->is_my_approval > 0 ? 'bg-yellow-50 dark:bg-yellow-900/10' : '' }}">
                        <td class="px-6 py-2 whitespace-nowrap text-xs">
                            <a href="{{ route('approval-ap-invoice.show', \Illuminate\Support\Facades\Crypt::encryptString($invoice->c_invoice_id)) }}"
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate"
                            title="{{ $invoice->description }}">{{ $invoice->description ?: '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ $invoice->bpartner_name ?? $invoice->c_bpartner_id ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-white">
                            {{ number_format($invoice->grandtotal, 2) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ date('d M Y', strtotime($invoice->dateinvoiced)) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            @php
                                $statusConfig = config('idempiere.approval-ap-invoice.statuses');
                                $statusLabel = $statusConfig['labels'][$invoice->docstatus] ?? ($invoice->status_label ?? $invoice->docstatus);
