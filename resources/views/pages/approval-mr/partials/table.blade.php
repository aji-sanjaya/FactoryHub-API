<div
        ...
        <table class="w-full table-auto">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($receipts as $receipt)
                    <tr
                        class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors {{ isset($receipt->is_my_approval) && $receipt->is_my_approval > 0 ? 'bg-yellow-50 dark:bg-yellow-900/10' : '' }}">
                        <td class="px-6 py-2 whitespace-nowrap text-xs">
                            <a href="{{ route('approval-mr.show', \Illuminate\Support\Facades\Crypt::encryptString($receipt->m_inout_id)) }}"
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate"
                            title="{{ $receipt->description }}">{{ $receipt->description ?: '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ $receipt->bpartner_name ?? $receipt->c_bpartner_id ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-white">
                            {{ number_format($receipt->grandtotal, 2) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ date('d M Y', strtotime($receipt->movementdate)) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            @php
                                $statusConfig = config('idempiere.approval-mr.statuses');
                                $statusLabel = $statusConfig['labels'][$receipt->docstatus] ?? ($receipt->status_label ?? $receipt->docstatus);
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
        {{ $receipts->appends(request()->query())->links('vendor.pagination.simple-tailwind') }}
    </div>
</div>