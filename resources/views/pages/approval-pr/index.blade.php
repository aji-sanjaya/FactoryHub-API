    ...
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav>
                <ol class="flex items-center gap-2">
                    ...
                    <li class="font-medium text-primary">Approval PR</li>
                </ol>
            </nav>
        </div>
        ...
        <div
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Cost Center</label>
                    <select id="costCenterFilter" class="select2 w-full">
                        @if($selectedCostCenter)
                            <option value="{{ $selectedCostCenter->id }}" selected>{{ $selectedCostCenter->text }}</option>
                        @endif
                    </select>
                </div>
        ...
        <div id="table-container" class="relative min-h-[400px]">
            ...
            <div id="table-content">
                @include('pages.approval-pr.partials.table', ['requisitions' => $requisitions])
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (typeof $ !== 'undefined') {
                ...
                $('#supplierFilter').select2({
                    ajax: {
                        url: '{{ route("approval-pr.suppliers") }}',
                        dataType: 'json',
                        delay: 250,
                ...
                $('#costCenterFilter').select2({
                    ajax: {
                        url: '{{ route("approval-pr.cost-centers") }}',
                        dataType: 'json',
                        delay: 250,
                        data: function (params) {
                            return {
                ...
                $('#statusFilter, #supplierFilter, #costCenterFilter').on('change', function () {
                    fetchData();
                });
            } else {
        ...
        // Removed setStatus function as it is replaced by Select2 change event

        // Filters - handled by Select2 change event above
        // document.getElementById('supplierFilter').addEventListener('change', () => fetchData());
        // document.getElementById('costCenterFilter').addEventListener('change', () => fetchData());
        ...
        function fetchData(page = 1) {
            ...
            const newUrl = "{{ route('approval-pr.index') }}?" + params.toString();
