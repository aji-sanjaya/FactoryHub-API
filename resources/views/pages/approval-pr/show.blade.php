...
@section('content')
            ...
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <h2 class="text-title-md2 font-bold text-black dark:text-white">
                        PR: {{ $requisition->documentno }}
                    </h2>
                </div>
                <div class="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400 ml-7">
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-medium 
                                                {{ $requisition->docstatus == 'IP' ? 'bg-yellow-100 text-yellow-800' :
        ($requisition->docstatus == 'CO' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800') }}">
                        {{ $requisition->status_label }}
                    </span>
                    <span>{{ date('d M Y', strtotime($requisition->datedoc)) }}</span>
                    <span>{{ $requisition->description }}</span>
                </div>
            </div>

            <div class="flex items-center gap-3 ml-7 sm:ml-0">
                @if($requisition->docstatus == 'IP' && $isMyApproval)
            ...
            <div id="tab-content">
                ...
                <div id="document-panel" class="block">
                    <div class="p-0 overflow-hidden bg-gray-100 dark:bg-gray-800" style="height: calc(100vh - 250px);">
                        <iframe src="{{ route('requisition.print', $encryptedId) }}" class="w-full h-full border-0"
    ...
    <!-- Attachment Preview Modal (Required by tab-attachments logic) -->
...
@push('scripts')
    ...
    <script>
        ...
        function loadAttachments() {
            ...
            axios.get('{{ route("approval-pr.show", $encryptedId) }}', {
                ...
                .then(res => {
                    ...
                    // Re-bind scripts if needed? The partial likely has inline JS or relies on global functions.
                    // tab-attachments relies on toggleSelectAll, handleFileUpload (hidden here), openAttachmentPreview
                })
                .catch(err => {
                    console.error(err);
        ...
        function handleApproval(action) {
            Swal.fire({
                ...
                preConfirm: (comment) => {
                    return axios.post('{{ route("approval-pr.process", $encryptedId) }}', {
                        action: action,
                        comment: comment
                    })
                        .then(response => {
        ...
        // Attachment Preview Logic (Simplified from form.blade.php)
        ...
        // Stub for delete which might be called by partial but hidden/disabled
        window.toggleSelectAll = function () { };
        window.updateDeleteButtonState = function () { };
        window.deleteSelectedAttachments = function () { };
        // tab-attachments checks !$isReadOnly before showing delete buttons, so these shouldn't be called.

        // Explicitly set ReadOnly context for attachments? The partial handles it.
    </script>
@endpush