...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <!-- Page Header -->
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Petty Cash Request</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage petty cash requests</p>
        </div>
            ...
            <div class="flex items-center gap-3 relative">
                ...
                <div x-show="openFilter" @click.outside="openFilter = false"
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Date Range -->
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Date Range</label>
                    ...
                    <table class="w-full table-auto">
                        <thead class="bg-gray-50/50 dark:bg-gray-800/50">
                            <tr class="bg-gray-50 text-left dark:bg-white/[0.03]">
                                ...
                                <th class="min-w-[120px] px-4 py-3 font-medium text-gray-500 text-theme-xs dark:text-gray-400">
                                    DATE
                                </th>
                                <th class="min-w-[150px] px-4 py-3 font-medium text-gray-500 text-theme-xs dark:text-gray-400">
                                    USER
                            ...
                            @forelse ($requests ?? [] as $index => $request)
                                <tr class="border-t border-gray-200 dark:border-gray-800">
                                    <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                        {{ $index + 1 + (($requests->currentPage() - 1) * $requests->perPage()) }}
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                        {{ date('d M Y', strtotime($request->datetrx)) }}
                                    </td>
                                    <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                        {{ $request->user_name ?? '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                        <div class="truncate max-w-xs whitespace-nowrap" title="{{ $request->description }}">
                                            {{ $request->description ?: '-' }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-gray-800 text-theme-sm dark:text-white/90 text-right font-medium">
                                        {{ number_format($request->totallines ?? 0, 2) }}
                                    </td>
                                    <td class="px-4 py-3 text-right">
                                        @php
                                            $statusLabel = $request->status_label ?? 'Draft';
                                ...
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-base font-medium">No petty cash requests found</p>
                ...
                @if(isset($requests) && $requests->total() > 0)
                    <div class="px-4 py-4 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            Showing <span class="font-medium text-gray-900 dark:text-white">{{ $requests->firstItem() ?? 0 }}</span> 
                            to <span class="font-medium text-gray-900 dark:text-white">{{ $requests->lastItem() ?? 0 }}</span> 
                            of <span class="font-medium text-gray-900 dark:text-white">{{ $requests->total() }}</span> results
                        </div>
                        <div class="w-full sm:w-auto">
                            {{ $requests->onEachSide(1)->links('pagination::tailwind') }}
                        </div>
                    </div>
                @endif
            </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const tableWrapper = document.getElementById('request-table-wrapper');

                let debounceTimer;
                let currentFilters = {};

                // Function to fetch data
                const fetchData = (url = '{{ route("petty-cash-request.index") }}') => {
                    const query = searchInput.value;
                    const wrapper = document.getElementById('request-table-wrapper');
