...
@section('content')
    ...
    @php 
        ...
        // Force active tab to header on page load/refresh
        $activeTab = 'header';
        ...
        $docNo = $isNew ? '** New **' : $requisition->documentno;
        $status = $isNew ? $requisitionConfig['defaults']['document_status_label'] : $requisition->status_label;
        $desc = $isNew ? '' : $requisition->description;
        ...
        $warehouseId = $isNew ? $defaultWarehouseId : $requisition->m_warehouse_id;

        // Current Org ID Selection
        $currentOrgId = $isNew ? $defaultOrgId : $requisition->ad_org_id;
        $dateRequired = $isNew ? now()->format('m-d-Y') : \Carbon\Carbon::parse($requisition->daterequired)->format('m-d-Y');
        ...
        // Read Only Logic
        $isReadOnly = false;
        if (!$isNew && isset($requisition->docstatus)) {
            $isReadOnly = in_array($requisition->docstatus, $requisitionConfig['statuses']['read_only'], true);
        }

        $printableStatuses = $requisitionConfig['statuses']['printable'] ?? [];
        $workflowConfig = $requisitionConfig['workflow'] ?? [];
        $reactivateStatuses = $workflowConfig['reactivate_from'] ?? [];
        $standardStatuses = $workflowConfig['standard_from'] ?? [];
        ...
        $currentDocStatus = !$isNew && isset($requisition->docstatus) ? $requisition->docstatus : 'DR';
        $canReactivate = $currentDocStatus === \App\Models\Idempiere\MRequisition::STATUS_COMPLETED
            || in_array($currentDocStatus, $reactivateStatuses, true);
    @endphp
    ...
    <div>
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                        ...
                        @endif
                        <span>{{ $isNew ? 'Create a new purchase requisition details' : 'Manage requisition details and lines' }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                @if(isset($requisition) && in_array($requisition->docstatus, $printableStatuses, true))
                    <button type="button"
                        onclick="openPrintModal('{{ route('requisition.print', \Illuminate\Support\Facades\Crypt::encryptString($requisition->m_requisition_id)) }}')"
                ...
                <div id="header-actions" class="{{ $activeTab == 'header' ? 'block' : 'hidden' }} flex gap-3">
                    @if(!$isReadOnly)
        ...
        <div
            ...
            <div id="tab-content-wrapper">
                ...
                <div id="tab-header" class="{{ $activeTab == 'header' ? 'block' : 'hidden' }}">
                    @include('pages.requisition.partials.tab-header')
                </div>

                <!-- Lines Content -->
                <div id="tab-lines" class="{{ $activeTab == 'lines' ? 'block' : 'hidden' }}">
                    @include('pages.requisition.partials.tab-lines')
                </div>
    ...
    @push('scripts')
        ...
        <script>
            const REQUISITION_ACTION_LABELS = @json($workflowActionLabels);
            const REQUISITION_ACTION_CONFIRMATIONS = @json($workflowConfirmationMessages);
            ...
            function initScripts() {
                    ...
                    $('#m_product_id').off('change').on('change', function () {
                        ...
                        if (selectedData && selectedData.length > 0) {
                            ...
                            if (productId && priceListId) {
                                axios.get('{{ route("requisition.api.product-price") }}', {
                                    params: {
                ...
                $('#org_id').off('change').on('change', function () {
                    ...
                    axios.get('{{ route("requisition.api.warehouses") }}', { params: { org_id: orgId } })
                        .then(res => {
                            const data = res.data;
                            let minId = null;
            ...
            function performDelete(lineIds) {
                ...
                axios.delete('{{ route("requisition.line.delete") }}', { data: data })
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
            ...
            function editLine(lineId, productText, productId, qty, price, description, lineNumber, uomName) {
                // Show the form
                showCreateLineForm();
                ...
                // Update form title (optional, if you have one)
                const formTitle = document.querySelector('#lineFormPanel h3');
                if (formTitle) {
            ...
            function saveLine() {
                ...
                const url = lineId ? '{{ route("requisition.line.update") }}' : '{{ route("requisition.line.store") }}';
            ...
            function submitHeader() {
                ...
                if (!data.date_required) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Validation Error',
                        text: 'Date Required is mandatory',
                        confirmButtonColor: '#4f46e5'
                    });
                    return;
                }
                ...
                const isNew = {{ $isNew ? 'true' : 'false' }};
                const reqId = {{ isset($requisition) ? $requisition->m_requisition_id : 'null' }};
                ...
                if (isNew) {
                    axios.post('{{ route("requisition.store") }}', data)
                        .then(res => {
                            Swal.close();
                            if (res.data && res.data.data && res.data.data.encrypted_id) {
                                window.location.href = "{{ route('requisition.index') }}?document_id=" + res.data.data.encrypted_id;
                            } else {
                    ...
                    let updateUrl = '{{ route("requisition.update", "ID_PLACEHOLDER") }}'.replace('ID_PLACEHOLDER', reqId);
            ...
            function executeDocumentAction(action) {
                ...
                actionText = REQUISITION_ACTION_LABELS[action] || action;
                confirmText = REQUISITION_ACTION_CONFIRMATIONS[action] || 'Are you sure you want to process this document?';
            ...
            function performDocumentAction(action, actionText) {
                ...
                axios.post('{{ route("requisition.process") }}', data)
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            ...
                            window.location.href = "{{ route('requisition.index') }}?document_id={{ $docIdParam }}&tab=header";
                        });
                    })
                    .catch(err => {
                        console.error(err);
            ...
            function deleteRequisition() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        performDeleteRequisition();
                    }
                });
            }

            function performDeleteRequisition() {
                // Show loading
                Swal.fire({
                    title: 'Deleting...',
                    text: 'Please wait',
                ...
                axios.delete('{{ route("requisition.delete") }}', { data: data })
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: res.data.message || 'Requisition deleted successfully',
                            confirmButtonColor: '#4f46e5',
                            timer: 2000
                        }).then(() => {
                            // Redirect to list
                    ...
                    .catch(err => {
                        ...
                        const cannotDelete = err.response?.status === 422;

                        Swal.fire({
                            icon: cannotDelete ? 'warning' : 'error',
                            title: cannotDelete ? 'Cannot Delete' : 'Error',
                            text: cannotDelete
                                ? errorMessage
                                : 'Error deleting requisition: ' + errorMessage,
                            confirmButtonColor: '#4f46e5'
                        });
                    });
            }
            ...
            window.uploadSingleFile = function (file) {
                ...
                axios.post('{{ route("requisition.attachment.upload") }}', formData, {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const promises = ids.map(id => {
                            return axios.delete('{{ route("requisition.attachment.delete") }}', {
                                data: {
                                    document_id: docId,
                                    attachment_id: id
                                }
            ...
            window.deleteAttachment = function (attId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.delete('{{ route("requisition.attachment.delete") }}', {
                            data: {
                                document_id: docId,
                                attachment_id: attId
                            }
            ...
            <div class="mt-4 space-y-3">
                @if($canReactivate)
                    ...
                    <button type="button" onclick="executeDocumentAction('{{ $reactivateAction }}')"
                        class="w-full flex items-center justify-between px-4 py-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg transition-colors dark:bg-gray-900/20 dark:hover:bg-gray-900/30 dark:border-gray-800">
                        <div class="flex items-center gap-3">
                            <svg class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 4v5h.582m14.836 2A8.001 8.001 0 005.582 9m0 0H9m11 11v-5h-.581m0 0A8.003 8.003 0 016.582 15m12.837 0H15"></path>
                            </svg>
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $workflowActionLabels[$reactivateAction] ?? 'Re-Active' }}</span>
                        </div>
                        <svg class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                ...
                @elseif(in_array($currentDocStatus, $standardStatuses, true))
                    <!-- Standard Actions for Draft/In Progress -->
