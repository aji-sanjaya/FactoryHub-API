...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <!-- Summary Cards -->
        <x-requisition.summary-cards :countAll="$countAll" :countDraft="$countDraft" :countInProgress="$countInProgress"
            :countCompleted="$countCompleted" />

        <!-- Action / Filter Bar -->
        <x-requisition.filter-bar />

        <!-- Table -->
        <div id="requisition-table-wrapper">
            <x-requisition.requisition-table :requisitions="$requisitions" />
        </div>

        <!-- Create Modal -->
        <x-requisition.create-modal />
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const fetchData = (url = '{{ route("requisition.index") }}') => {
