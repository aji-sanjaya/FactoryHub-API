...
@section('content')
    ...
    @php 
        ...
        $warehouseId = $isNew ? $defaultWarehouseId : $salesOrder->m_warehouse_id;

        // Current Org ID Selection
        $currentOrgId = $isNew ? $defaultOrgId : $salesOrder->ad_org_id;
        $dateRequired = $isNew ? now()->format('m-d-Y') : \Carbon\Carbon::parse($salesOrder->daterequired)->format('m-d-Y');
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                        ...
                        @endif
                        <span>{{ $isNew ? 'Create a new purchase sales-order details' : 'Manage sales-order details and lines' }}</span>
                    </p>
                </div>
            </div>
                ...
                <div id="header-actions" class="{{ $activeTab == 'header' ? 'block' : 'hidden' }}">
                    @if(!$isReadOnly)
                        <button onclick="submitHeader()"
                            ...
                            {{ $isNew ? 'Create Sales Order' : 'Save Changes' }}
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div
            ...
            <div id="tab-content-wrapper">
                ...
                <div id="tab-header" class="{{ $activeTab == 'header' ? 'block' : 'hidden' }}">
                    @include('pages.sales-order.partials.tab-header')
                </div>

                <!-- Lines Content -->
                <div id="tab-lines" class="{{ $activeTab == 'lines' ? 'block' : 'hidden' }}">
                    @include('pages.sales-order.partials.tab-lines')
                </div>
        ...
        <script>
            ...
            function initScripts() {
                ...
                $('#c_bpartner_id').off('change.header').on('change.header', function () {
                    ...
                    axios.get('/sales-order-api/bpartner-locations', { params: { bpartner_id: bpartnerId } })
                        .then(res => {
                ...
                if ($('#m_product_id').length > 0) {
                    $('#m_product_id').select2({
                        ...
                        allowClear: false,
                        ajax: {
                            url: '{{ route("sales-order.api.products") }}',
                            dataType: 'json',
                            delay: 250,
                    ...
                    $('#m_product_id').off('change').on('change', function () {
                        ...
                        if (selectedData && selectedData.length > 0) {
                            ...
                            if (productId && priceListId) {
                                axios.get('{{ route("sales-order.api.product-price") }}', {
                                    params: {
                ...
                $('#org_id').off('change').on('change', function () {
                    ...
                    axios.get('{{ route("sales-order.api.warehouses") }}', { params: { org_id: orgId } })
                        .then(res => {
                            const data = res.data;
                            let minId = null;
            ...
            function performDelete(lineIds) {
                ...
                axios.delete('{{ route("sales-order.line.delete") }}', { data: data })
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
            ...
            function editLine(lineId, productText, productId, qty, price, description, lineNumber, uomName, uomPrecision, qtyDelivered, qtyInvoiced, datePromised) {
                // Show the form
                showCreateLineForm();
            ...
            function saveLine() {
                ...
                const url = lineId ? '{{ route("sales-order.line.update") }}' : '{{ route("sales-order.line.store") }}';
            ...
            function submitHeader() {
                ...
                const isNew = {{ $isNew ? 'true' : 'false' }};
                const reqId = {{ isset($salesOrder) ? $salesOrder->c_order_id : 'null' }};
                ...
                if (isNew) {
                    axios.post('{{ route("sales-order.store") }}', data)
                        .then(res => {
                            Swal.close();
                            if (res.data && res.data.data && res.data.data.encrypted_id) {
                                window.location.href = "{{ route('sales-order.index') }}?document_id=" + res.data.data.encrypted_id;
                            } else {
                    ...
                    let updateUrl = '{{ route("sales-order.update", "ID_PLACEHOLDER") }}'.replace('ID_PLACEHOLDER', reqId);
            ...
            function performDocumentAction(action, actionText) {
                ...
                axios.post('{{ route("sales-order.process") }}', data)
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            ...
                            window.location.href = "{{ route('sales-order.index') }}?document_id={{ $docIdParam }}&tab=header";
                        });
                    })
                    .catch(err => {
                        console.error(err);
            ...
            function deleteSalesOrder() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        performDeleteSalesOrder();
                    }
                });
            }

            function performDeleteSalesOrder() {
                // Show loading
                Swal.fire({
                    title: 'Deleting...',
                    text: 'Please wait',
                ...
                axios.delete('{{ route("sales-order.delete") }}', { data: data })
                    .then(res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: res.data.message || 'Sales Order deleted successfully',
                            confirmButtonColor: '#4f46e5',
                            timer: 2000
                        }).then(() => {
                            // Redirect to list
                    ...
                    .catch(err => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error deleting sales-order: ' + (err.response?.data?.message || err.message),
                            confirmButtonColor: '#4f46e5'
                        });
                    });
            }
            ...
            window.uploadSingleFile = function (file) {
                ...
                axios.post('{{ route("sales-order.attachment.upload") }}', formData, {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const promises = ids.map(id => {
                            return axios.delete('{{ route("sales-order.attachment.delete") }}', {
                                data: {
                                    document_id: docId,
                                    attachment_id: id
                                }
            ...
            window.deleteAttachment = function (attId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.delete('{{ route("sales-order.attachment.delete") }}', {
                            data: {
                                document_id: docId,
                                attachment_id: attId
                            }
